      ...
      <div className="container">
        <div className="row">
          ...
          <div className="col-11">
            ...
            <p className="blog-body">Who’s Right was a website I created as a fun social app that allowed users to post their opinions or thoughts, and then let other people vote on them to see how popular their thought was. I thought it would be a good way to spark debates and discussions online and help people connect with one another through ideas. Funnily enough, there was already a popular website that was established a long time ago called debate.org that a hardcore debate club friend of mine pointed out to me. Oh well, I still think I made something pretty cool.</p>
            <p className="blog-body">This project was the first full stack application I built using the MERN stack, and I believe I accomplished a lot by putting it together. I had created profile pictures to show posts that people already voted on and their own posts, managed to limit users to one vote per account, and created a Youtube-like  “like bar” to visualize vote data. I also created a comment section where people could have discussions on the topics posted.</p>
            <div>
              <h3 className="subheader">Building the site</h3>
              <p className="blog-body">I began the front end by creating the “like bar”. This was done by creating an underlying div that had a red background color, and then placing a green background color div on top of it. The width of the green div was based on the Redux state put to local state from the backend API.</p>
              <img className="scrnshot" src="./assets/whosright-pics/postvote.png"/>
              <p className="blog-body">After that I created the buttons to actually vote on the post. I rendered the proper components (buttons or “like bar” and comments) from local state that checked with the backend to see if the user had the post id saved under their user object in the database. </p>
              <img className="scrnshot" src="./assets/whosright-pics/buttons.png"/>
              <p className="blog-body">After finishing these components, I made the form to create a post, which was handled with Redux Form to easily handle input changes in the React front-end. Interestingly enough, I decided to create the category selectors with checkboxes styled as buttons. I realize now that was probably not the best approach. I worked around the boolean return value by creating a separate component for each category, passing the category name as a prop. Within the component I created an onChange event handler function that would add or remove the element from the array using the built-in javascript array concat and filter methods. This array would then be submitted along with the other values to the back-end creating a post with three tags.</p>
